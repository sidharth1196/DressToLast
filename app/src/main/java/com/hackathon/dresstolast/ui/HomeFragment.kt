    ...
    private fun initObservers() {
        viewModel.brands.observe(viewLifecycleOwner, Observer {
            it.forEach {
                ...
                val logoUrl = when {
                    it.name == "H&M" -> "photo_hm"
                    else -> "photo_${it.name.split(" ").joinToString("").toLowerCase()}"
                }
                Log.d("DTL", "logo name url : $logoUrl")
                it.brandLogo = resources.getIdentifier(logoUrl, "drawable", context?.packageName)
            }
